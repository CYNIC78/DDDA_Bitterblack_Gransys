...
void AnimTick()
{
    ...
    for (int i = 0; i < g_animCount; ++i) {
        ...
        if (g_animScope == kScopeAttack && !t.viaOverride) {
            ...
            // Исключение для многоударных быстрых рашей (SwMoveAttack)
            // Анимация сабельного раша уже быстрая от природы; ускорение превращает ее в глитч.
            if (t.actIsAttack && factor > 1.0f && act) {
                char nm[48] = {};
                if (Mem::NameOfLiveObjectSafe((const void*)act, nm, sizeof(nm))) {
                    if (strstr(nm, "SwMoveAttack") != nullptr) {
                        factor = 1.0f;
                    }
                }
            }
