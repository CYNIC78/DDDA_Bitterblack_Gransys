...
#include <stdint.h>
#include "MemProbe.h"
// Публичный контракт рантайма тянем сюда намеренно.
//
// Дважды подряд получили одну и ту же ошибку сборки: модуль включает
// ВНУТРЕННИЙ заголовок, зовёт функцию из ПУБЛИЧНОГО и падает на
// «не является членом Runtime». Внутренний контракт — надстройка над
// публичным, а не альтернатива ему, поэтому пусть тянет его сам.
#include "Runtime.h"
