/**
 * DDDA AI Overhaul Mod
 * Основан на архитектуре ddda-dinput8 by kubik-jaroslav
 * Цель: улучшение AI пешек и монстров без читов
 */
...
void InitHooks()
{
    // Находим базовые указатели (как в оригинальном dinput8)
    ...
    // Инициализируем ImGui-оверлей (F12 для открытия меню)
    if (Hooks::InGameUI())
    {
        // Наши модули регистрируют свой UI через InGameUIAdd()
        Hooks::CombatIntel();
        Hooks::PawnAI();
        Hooks::EnemyAI();
        Hooks::Nightmare();
    ...
    {
        logFile << "InGameUI: failed to initialize. F12 menu won't work." << std::endl;
    }
}
...
void Initialize()
{
    ...
    logFile << "DDDA AI Overhaul - Initialized successfully!" << std::endl;
}
