...
## 7. Упаковка билда в zip (source package)

Каждый собранный билд упаковывается в zip и кладётся в `builds/` в корне
репозитория. Скачал архив → распаковал → открыл `.sln` → собрал. Ничего
дополнительно не качать: `MinHook/`, `ImGui/`, ini и ресурсы уже внутри.

**Имя архива** — краткое, с префиксом номера билда: `{MOD_BUILD_TAG}.zip`
(например, `84.65-garm-bestiary.zip`). Вручную не переименовываем.

```bash
python3 tools/package_build.py
```
...
- **перед упаковкой** поднять `MOD_BUILD_TAG` (`src/BuildTag.h`) и записать
  изменения в `CHANGELOG.md` → секция «Текущий milestone» — её текст и
  попадёт в `MANIFEST.txt`;
- zip в `builds/` **не коммитим** (`builds/*.zip` в `.gitignore`) — это
  артефакт для скачивания, а не исходник;
- тег сборки — единственный идентификатор: первая строка лога печатает
  `MOD_BUILD_TAG`, по нему сверяем, что игра запустила именно эту DLL.
