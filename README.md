...
## 🎯 Текущий Milestone: Build 84.62 (`84.62-dual-guardian-nexus-synergy`)

Синхронная работа Guardian (защита Аризена) + Nexus (защита Чародейки) для всей партии:

1. **Синхронная коробка защиты (Dual-Bodyguard Formation)**:
...
2. **Устранение эффекта "Бенни Хилла" на сабельном раше (`SwMoveAttack`)**:
   - Атака непрерывного бега со взмахами саблей (`cEm0100ActSwMoveAttack`) исключена из ускорения темпа (`factor = 1.0f`), возвращая естественную кинематику и вес ударов.
3. **Гибридный Nuke Gating (Угроза в упор + Одиночный моб)**:
   - **Защита от суицида в упор (< 6.0 м)**: блокировка 15с спеллов при угрозе в упор.
   - **Свобода зачистки сцены (Crowd Wipe)**: зрелищные метеориты и смерчи на пачках и боссах.
4. **Оптимизация дистанции HFB (`RangeMinXZ = 4.5 м`)**:
   - Минимальная планка зарядки Focused Bolt установлена на 4.5 метра (`4.5..20 м`).
5. **Общепартийное спасение Аризена (Party Emergency Rescue Protocol)**:
   - Автоматическая мобилизация всех пешек (Главная + Наёмники) при захвате/нокдауне Аризена со скоростным рывком (x1.30) и принудительным фокусом.
6. **DDON Return Sanctuary**:
