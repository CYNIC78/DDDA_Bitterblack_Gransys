...
**Текущий milestone:** tag `84.44-genetic-scale` — генетический видовой масштаб
особей стаи (`SpeciesCard` + `EnemyTuner`): безопасные коридоры для гоблинов (0.93..1.15),
хобов (0.95..1.14), волков (0.94..1.10) и ящеров (0.95..1.12); защита авторских
альфа-вожаков Capcom (`LEADER_PRESERVED`); компенсация частоты шага в `MonsterTempo`
(`cadence = 1.0 / scale`). Ранее: `84.43-true-torpor` (истинный Торпор id=1 и Debilitation Lab);
`84.40-caster-harass` (прессинг Аризена-кастера); `84.39-caller-defense` (защита горниста);
`84.38-true-bestiary` (полная синхронизация 72 видов, Григори uEm5800, снятие босс-статуса с зайцев).
