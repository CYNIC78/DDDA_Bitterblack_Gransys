...
счётчик архивов в доках исправлен 95 → **97**; intake-скрипт очищен от
старого «сдвига» и защищён от сбойных файлов.

**84.66** — полный список `nativePC/rom/enemy/` (97 архивов) сверен с
`types.tsv`: **номер архива = номер DTI** для всех семейств. Исправлен
мнимый «сдвиг» в SoT §8.5.1/§8.5.2 и ошибочный блок `ARC_MAP.txt`.

**84.65** — Гарм (`uEm0204`) возвращён в бестиарий (делил слот 52 с Варгом и
терялся при генерации); исправлены перепутанные подписи видов в
`ddda_entities.ini` (Хобгоблин/Гримгоблин) по фактам `*.rst`/`*.prp`/`*.sn2`;
упаковка билда в zip автоматизирована (`tools/package_build.py`).

### 84.64 — `tactical-dpad-commands` (предыдущий)
...
Готовый билд пакуется в zip в папку `builds/`, имя архива = тег сборки
(`{MOD_BUILD_TAG}.zip`). Одна команда: `python3 tools/package_build.py`.
Подробно — [`docs/BUILD_INSTRUCTIONS_RU.md`](docs/BUILD_INSTRUCTIONS_RU.md) §7.
