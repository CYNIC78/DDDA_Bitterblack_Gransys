...
## 🎯 Текущий Milestone: Build 84.55 (`84.55-pawn-combat-mastery`)

Комплексный пакет улучшений боевого поведения пешек и монстров:

1. **Двухуровневый телохранитель (Two-Tier GuardianDoctrine)**:
   - **Critical Melee (4–6 м)**: жесткий перехват любой цели вплотную к Аризену (`uCmc + 0x2EB8`) с максимальным боевым приоритетом.
   - **Preempt Intercept (6–12 м)**: упреждающий перехват врагов, нацеленных на Аризена (`targetMember == MEMBER_ARISEN`) или выполняющих атаку. Если враг фланкирует другую пешку, Гвардиан сохраняет строй рядом с Аризеном.
   - **Поводок безопасности (leashDistance 14–18 м)**: исключает бесконечную погоню и побеги пешки на край карты.
2. **Общепартийное спасение Аризена (Party Emergency Rescue Protocol)**:
   - Автоматическая мобилизация всех пешек (Главная + Наёмники) при захвате/нокдауне Аризена (огры `Catch/Eat`, гримгоблины `TramplePl`, гарпии `CarryCatchPl`, циклопы, драконы).
   - Принудительный фокус цели (`uCmc + 0x2EB8`), снятие штрафов пассивности и скоростной рывок (x1.30) для немедленного сбивания захвата.
3. **Умный кастер и разблокировка Focused Bolt (Caster Tactics & WandRange)**:
   - **Smart Nuke Gating**: блокировка 15-секундных кастов (Bolide, Maelstrom, Seism, Fulmination) на обычных мобах с переключением на быстрые спеллы (Comestion, Ingle, Levin, Frigor, Focused Bolt).
   - **Разблокировка на Боссах**: мгновенный допуск ядерных заклинаний (15 м) против крупных монстров (Огры, Циклопы, Химеры, Дрейки, Големы, Даймон).
   - **Focused Bolt (`cCmcWandDXL`)**: дальность заряженной атаки расширена с 2 до 15–20 метров.
4. **DDON Return Sanctuary**:
   - 4-кратная броня и x1.30 скорость отхода на спавн у отступающих монстров.
5. **Оптимизация производительности**:
   - Zero-allocation буфер лога + аварийный сброс VEH при любых сбоях.
   - Кэширование структур EnemyTuner (устранено 150 000 чтений/сек) и чистый лог без спама.
